...
fn copy_one_bom_file(bom_file_path: &str) {
    ...
    for (from, destination) in copy_dirs.iter() {
        ...
        if !destination.is_dir() {
            panic!("Copy {:?} to {:?} failed.", from, destination);
        }
    }
    ...
    for (from, destination) in copy_files.iter() {
        let destination = PathBuf::from(destination);
        let destination_directory = destination.parent().unwrap();
        //if the directory where we place files does not exit, create the directory first
        std::fs::create_dir_all(destination_directory)
            .expect(format!("Create directory {:?} failed.", destination_directory).as_str());
