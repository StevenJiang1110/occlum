...
use std::io::Write;
use std::str::FromStr;
use std::{collections::HashMap, io::Read, fs};
...
impl TraceOption {
    fn parse_option() -> Self {
        let matched = App::new("occlum-trace")
            ...
            .arg(Arg::new("trace-filename")
                .short('i')
                .long("input")
                .about("the input trace file name(required)")
                .required(true)
                .takes_value(true)
            )
            .arg(Arg::new("output-filename")
                .short('o')
                .long("output")
                .about("the output file name(required)")
                .required(true)
                .takes_value(true)
            )
            .arg(Arg::new("filter-syscall-name")
                .short('f')
                .long("filter")
                .about("syscall names to be filtered (case intensive). Example: -f Ioctl,Futex,Madvise")
                .takes_value(true)
            )
            .get_matches();
        ...
        if log.level_above_info() {
            ...
            if let Some(matched_entry) = find_matched_str(&log.log_content, &SYSCALL_ENTRY_REGEX, &mut 0){
                ...
                for ((thread_number,syscall_number), log) in &syscall_entry {
                    ...
                    let reduced_log_content = format!("{} <unfinished ...>", resumed_log.log_content);
                    resumed_log.log_content = reduced_log_content;
                ...
                if syscall_resume.contains(&(log.thread_number, log.syscall_number)) {
                    ...
                    let reduced_log_content = format!("<...resumed> = {}", return_value);
                    reduced_log.log_content = reduced_log_content;
                    ...
                    let reduced_log_content = format!("{} = {}", reduced_log.log_content, return_value);
                    reduced_log.log_content = reduced_log_content;
                    reduced_logs.push(reduced_log);
...
fn get_filename_from_full_path(matched_string: &str, regex_: &Regex) -> String{
    ...
    //println!("raw_filename = {:?}", raw_filename);
...
fn check_thread_exit_in_each_process(process_log: &Vec<OcclumLog>) {
    ...
    for log in process_log {
        if let Some(syscall_name) = &log.syscall_name {
            if syscall_name.as_str() == "ExitGroup" || syscall_name.as_str() == "Exit"{
